...
include 'db_conn.php';
    ...
    $sql = "SELECT itemList.id, name, price, type FROM items, itemList WHERE restaurant = '3' AND items.id = itemList.itemId order by type";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc())
            array_push($items, new item($row["id"], $row["name"], $row["price"], $row["type"], '3'));
    
    }
    
    $sql = "SELECT openTime, closeTime FROM restaurant WHERE id = '3'";
    $result = $mysqli->query($sql);
    while($row = $result->fetch_assoc()){
