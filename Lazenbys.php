...
<?php
    ...
    $sql = "SELECT itemList.id, name, price, type FROM items, itemList WHERE restaurant = '1' AND items.id = itemList.itemId order by type";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc())
            array_push($items, new item($row["id"], $row["name"], $row["price"], $row["type"], '1'));
    
    }
    
    $sql = "SELECT openTime, closeTime FROM restaurant WHERE id = '1'";
    $result = $mysqli->query($sql);
    while($row = $result->fetch_assoc()){
...
<script>
    ...
    var orderList = [];
    var total = 0;
    var openTime = <?php echo json_encode($openTime); ?>.split(":");
    var closeTime = <?php echo json_encode($closeTime); ?>.split(":");
